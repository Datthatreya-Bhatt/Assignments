...
window.onload = async()=>{
    try{
        let res = await axios.get(`${url}/download/list/1/pagelimit?limit=${5}`);
        console.log(res);
        loop(res);
...
async function prev(){
    ...
    if(curr>1){
        ...
        try{
            ...
            let page = curr-1;
            let res = await axios.get(`${url}/download/list/${page}/pagelimit?limit=${limit}`);
           // pagination(res);
            loop(res);
...
function loop(res){
    ...
    if(length>0){
        ...
        for(let i = 0;i<length;i++){
            let links = res.data.user[i].links;
